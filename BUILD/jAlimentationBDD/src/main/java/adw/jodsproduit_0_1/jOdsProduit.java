...
public static class row_Implicit_Context_DatabaseStruct implements routines.system.IPersistableRow<row_Implicit_Context_DatabaseStruct> {
    final static byte[] commonByteArrayLock_ADW_jOdsProduit = new byte[0];
    static byte[] commonByteArray_ADW_jOdsProduit = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ADW_jOdsProduit.length) {
				if(length < 1024 && commonByteArray_ADW_jOdsProduit.length == 0) {
   					commonByteArray_ADW_jOdsProduit = new byte[1024];
				} else {
   					commonByteArray_ADW_jOdsProduit = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ADW_jOdsProduit, 0, length);
			strReturn = new String(commonByteArray_ADW_jOdsProduit, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_ADW_jOdsProduit, 0, length);
			strReturn = new String(commonByteArray_ADW_jOdsProduit, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ADW_jOdsProduit) {
 ...
 */
	...
	final String decryptedPassword_Implicit_Context_Database = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:qnUE59mUTgjVdPR9JwizZc3oLJcTuWSHxhljsPJcga/67lTo");
...
if(produit != null) { 
    ...
    Operator_tFilterRow_1 ope_tFilterRow_1 = new Operator_tFilterRow_1("&&");
      ope_tFilterRow_1.matches((! produit.CD_PRODUIT.isEmpty() && ! produit.NOM_PRODUIT.isEmpty() &&
! produit.CD_SOUS_CATEGORIE.isEmpty() &&
produit.PRIX_ACHAT_PRODUIT != null &&
produit.PRIX_VENTE_PRODUIT != null), "advanced condition failed");
      ...
      nb_line_ok_tFilterRow_1++;
            ...
            if (row6 == null){
              row6 = new row6Struct();
                ...
                row6.CD_PRODUIT = produit.CD_PRODUIT;
                row6.NOM_PRODUIT = produit.NOM_PRODUIT;
                row6.PRIX_ACHAT_PRODUIT = produit.PRIX_ACHAT_PRODUIT;
                row6.PRIX_VENTE_PRODUIT = produit.PRIX_VENTE_PRODUIT;
                row6.CD_SOUS_CATEGORIE = produit.CD_SOUS_CATEGORIE;
                row6.NUM_LIGNE = produit.NUM_LIGNE;
...
if(row7 == null){ 
	...
	row7 = new row7Struct();
}				row7.CD_PRODUIT = row2.CD_PRODUIT;			row7.NOM_PRODUIT = row2.NOM_PRODUIT;			row7.PRIX_ACHAT_PRODUIT = row2.PRIX_ACHAT_PRODUIT;			row7.PRIX_VENTE_PRODUIT = row2.PRIX_VENTE_PRODUIT;			row7.CD_SOUS_CATEGORIE = row2.CD_SOUS_CATEGORIE;			row7.NUM_LIGNE = row2.NUM_LIGNE;			
...
produits_tmp.NUM_LIGNE = row3.NUM_LIGNE ;
...
sousCategorieManquant_tmp.LB_LIGNE_REJET = row3.CD_PRODUIT +"|" + row3.NOM_PRODUIT ;
sousCategorieManquant_tmp.LB_MESSAGE_REJET = "Dependance au SOUS_CATEGORIE : "+  row3.CD_SOUS_CATEGORIE  +" n'existe pas dans la table ODS_SOUS_CATEGORIE" ;
 					    ...
 					    boolean forceLooprow5 = false;
 							...
 							row5Struct row5ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_4 = false;
                        		    		    ...
                        		    		    row5HashKey.CD_PRODUIT = produits.CD_PRODUIT ;
								  ...
								  if(!tHash_Lookup_row5.hasNext()) { // G_TM_M_090
				           		...
				           		if(tHash_Lookup_row5 != null && tHash_Lookup_row5.getCount(row5HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row5' and it contains more one result from keys :  row5.CD_PRODUIT = '" + row5HashKey.CD_PRODUIT + "'");
								} // G 071
...
!produit.NOM_PRODUIT.equals(row5.NOM_PRODUIT) ||
!produit.CD_SOUS_CATEGORIE.equals(row5.CD_SOUS_CATEGORIE)
|| produit.PRIX_ACHAT_PRODUIT.compareTo(row5.PRIX_ACHAT_PRODUIT )!= 0
|| produit.PRIX_VENTE_PRODUIT.compareTo(row5.PRIX_VENTE_PRODUIT )!=0
...
updateProduit_tmp.LB_JOB_NAME = jobName;
...
} else {pstmt_tDBOutput_1.setString(6, insertProduit.LB_NOM_FICHIER);
}
...
} else {pstmt_tDBOutput_2.setString(1, updateProduit.NOM_PRODUIT);
}

                    pstmt_tDBOutput_2.setBigDecimal(2, updateProduit.PRIX_ACHAT_PRODUIT);

                    pstmt_tDBOutput_2.setBigDecimal(3, updateProduit.PRIX_VENTE_PRODUIT);

                    if(updateProduit.CD_SOUS_CATEGORIE == null) {
pstmt_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(4, updateProduit.CD_SOUS_CATEGORIE);
}

                    if(updateProduit.LB_NOM_FICHIER == null) {
pstmt_tDBOutput_2.setNull(5, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(5, updateProduit.LB_NOM_FICHIER);
}

                    if(updateProduit.DT_INSERTION != null) {
pstmt_tDBOutput_2.setTimestamp(6, new java.sql.Timestamp(updateProduit.DT_INSERTION.getTime()));
} else {
pstmt_tDBOutput_2.setNull(6, java.sql.Types.TIMESTAMP);
}

                    pstmt_tDBOutput_2.setInt(7, updateProduit.BL_LIGNE_ACTIVE);

                    if(updateProduit.LB_JOB_NAME == null) {
pstmt_tDBOutput_2.setNull(8, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(8, updateProduit.LB_JOB_NAME);
}

                    if(updateProduit.CD_PRODUIT == null) {
pstmt_tDBOutput_2.setNull(9 + count_tDBOutput_2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(9 + count_tDBOutput_2, updateProduit.CD_PRODUIT);
}
...
} else {pstmt_tDBOutput_3.setString(6, insertNewProduit.LB_NOM_FICHIER);
}

                    if(insertNewProduit.DT_INSERTION != null) {
pstmt_tDBOutput_3.setTimestamp(7, new java.sql.Timestamp(insertNewProduit.DT_INSERTION.getTime()));
} else {
pstmt_tDBOutput_3.setNull(7, java.sql.Types.TIMESTAMP);
}

                    pstmt_tDBOutput_3.setInt(8, insertNewProduit.BL_LIGNE_ACTIVE);

                    if(insertNewProduit.LB_JOB_NAME == null) {
pstmt_tDBOutput_3.setNull(9, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_3.setString(9, insertNewProduit.LB_JOB_NAME);
}
...
} else {pstmt_tDBOutput_6.setString(1, sousCategorieManquant.LB_CHEMIN_FICHIER);
}

                    if(sousCategorieManquant.LB_NOM_FICHIER == null) {
pstmt_tDBOutput_6.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_6.setString(2, sousCategorieManquant.LB_NOM_FICHIER);
}

                    pstmt_tDBOutput_6.setInt(3, sousCategorieManquant.NUM_LIGNE_REJET);

                    if(sousCategorieManquant.LB_LIGNE_REJET == null) {
pstmt_tDBOutput_6.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_6.setString(4, sousCategorieManquant.LB_LIGNE_REJET);
}
...
} else {pstmtUpdate_tDBOutput_6.setString(1, sousCategorieManquant.LB_MESSAGE_REJET);
}

                        if(sousCategorieManquant.LB_NOM_FLUX == null) {
pstmtUpdate_tDBOutput_6.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(2, sousCategorieManquant.LB_NOM_FLUX);
}

                        if(sousCategorieManquant.DT_REJET != null) {
pstmtUpdate_tDBOutput_6.setTimestamp(3, new java.sql.Timestamp(sousCategorieManquant.DT_REJET.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_6.setString(4, sousCategorieManquant.LB_JOB_NAME);
}

                        if(sousCategorieManquant.LB_CHEMIN_FICHIER == null) {
pstmtUpdate_tDBOutput_6.setNull(5 + count_tDBOutput_6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(5 + count_tDBOutput_6, sousCategorieManquant.LB_CHEMIN_FICHIER);
}

                        if(sousCategorieManquant.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_6.setNull(6 + count_tDBOutput_6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(6 + count_tDBOutput_6, sousCategorieManquant.LB_NOM_FICHIER);
}

                        pstmtUpdate_tDBOutput_6.setInt(7 + count_tDBOutput_6, sousCategorieManquant.NUM_LIGNE_REJET);

                        if(sousCategorieManquant.LB_LIGNE_REJET == null) {
pstmtUpdate_tDBOutput_6.setNull(8 + count_tDBOutput_6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(8 + count_tDBOutput_6, sousCategorieManquant.LB_LIGNE_REJET);
}
...
} else {pstmtInsert_tDBOutput_6.setString(1, sousCategorieManquant.LB_CHEMIN_FICHIER);
}

                        if(sousCategorieManquant.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_6.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(2, sousCategorieManquant.LB_NOM_FICHIER);
}

                        pstmtInsert_tDBOutput_6.setInt(3, sousCategorieManquant.NUM_LIGNE_REJET);

                        if(sousCategorieManquant.LB_LIGNE_REJET == null) {
pstmtInsert_tDBOutput_6.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(4, sousCategorieManquant.LB_LIGNE_REJET);
}

                        if(sousCategorieManquant.LB_MESSAGE_REJET == null) {
pstmtInsert_tDBOutput_6.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(5, sousCategorieManquant.LB_MESSAGE_REJET);
}

                        if(sousCategorieManquant.LB_NOM_FLUX == null) {
pstmtInsert_tDBOutput_6.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(6, sousCategorieManquant.LB_NOM_FLUX);
}

                        if(sousCategorieManquant.DT_REJET != null) {
pstmtInsert_tDBOutput_6.setTimestamp(7, new java.sql.Timestamp(sousCategorieManquant.DT_REJET.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_6.setString(8, sousCategorieManquant.LB_JOB_NAME);
}
...
insertDuplicate_tmp.LB_LIGNE_REJET = row7.CD_PRODUIT +'|'+ row7.NOM_PRODUIT ;
...
insertReject_tmp.LB_LIGNE_REJET = row6.CD_PRODUIT +'|'+ row6.NOM_PRODUIT ;
...
} else {pstmt_tDBOutput_5.setString(1, insertReject.LB_CHEMIN_FICHIER);
}

                    if(insertReject.LB_NOM_FICHIER == null) {
pstmt_tDBOutput_5.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_5.setString(2, insertReject.LB_NOM_FICHIER);
}

                    pstmt_tDBOutput_5.setInt(3, insertReject.NUM_LIGNE_REJET);

                    if(insertReject.LB_LIGNE_REJET == null) {
pstmt_tDBOutput_5.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_5.setString(4, insertReject.LB_LIGNE_REJET);
}
...
} else {pstmtUpdate_tDBOutput_5.setString(1, insertReject.LB_MESSAGE_REJET);
}

                        if(insertReject.LB_NOM_FLUX == null) {
pstmtUpdate_tDBOutput_5.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_5.setString(2, insertReject.LB_NOM_FLUX);
}

                        if(insertReject.DT_REJET != null) {
pstmtUpdate_tDBOutput_5.setTimestamp(3, new java.sql.Timestamp(insertReject.DT_REJET.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_5.setString(4, insertReject.LB_JOB_NAME);
}

                        if(insertReject.LB_CHEMIN_FICHIER == null) {
pstmtUpdate_tDBOutput_5.setNull(5 + count_tDBOutput_5, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_5.setString(5 + count_tDBOutput_5, insertReject.LB_CHEMIN_FICHIER);
}

                        if(insertReject.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_5.setNull(6 + count_tDBOutput_5, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_5.setString(6 + count_tDBOutput_5, insertReject.LB_NOM_FICHIER);
}

                        pstmtUpdate_tDBOutput_5.setInt(7 + count_tDBOutput_5, insertReject.NUM_LIGNE_REJET);

                        if(insertReject.LB_LIGNE_REJET == null) {
pstmtUpdate_tDBOutput_5.setNull(8 + count_tDBOutput_5, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_5.setString(8 + count_tDBOutput_5, insertReject.LB_LIGNE_REJET);
}
...
} else {pstmtInsert_tDBOutput_5.setString(1, insertReject.LB_CHEMIN_FICHIER);
}

                        if(insertReject.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_5.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_5.setString(2, insertReject.LB_NOM_FICHIER);
}

                        pstmtInsert_tDBOutput_5.setInt(3, insertReject.NUM_LIGNE_REJET);

                        if(insertReject.LB_LIGNE_REJET == null) {
pstmtInsert_tDBOutput_5.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_5.setString(4, insertReject.LB_LIGNE_REJET);
}

                        if(insertReject.LB_MESSAGE_REJET == null) {
pstmtInsert_tDBOutput_5.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_5.setString(5, insertReject.LB_MESSAGE_REJET);
}

                        if(insertReject.LB_NOM_FLUX == null) {
pstmtInsert_tDBOutput_5.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_5.setString(6, insertReject.LB_NOM_FLUX);
}

                        if(insertReject.DT_REJET != null) {
pstmtInsert_tDBOutput_5.setTimestamp(7, new java.sql.Timestamp(insertReject.DT_REJET.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_5.setString(8, insertReject.LB_JOB_NAME);
}
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row4";
		...
		int tos_count_tAdvancedHash_row4 = 0;
			   		...
			   		// linked node: tMap_3 - inputs:(row3,row4) outputs:(produits,sousCategorieManquant)
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row5";
		...
		int tos_count_tAdvancedHash_row5 = 0;
			   		...
			   		// linked node: tMap_4 - inputs:(produits,row5) outputs:(insertProduit,updateProduit,insertNewProduit)
 ...
 */
	...
	currentComponent="tDBInput_2";
		...
		int tos_count_tDBInput_2 = 0;
		    ...
		    while (rs_tDBInput_2.next()) {
		        nb_line_tDBInput_2++;
							...
							if(colQtyInRs_tDBInput_2 < 1) {
								row5.CD_PRODUIT = null;
							} else {
	                         		
        	row5.CD_PRODUIT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 1, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 2) {
								row5.NOM_PRODUIT = null;
							} else {
	                         		
        	row5.NOM_PRODUIT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 3) {
								row5.PRIX_ACHAT_PRODUIT = null;
							} else {
		                          
            row5.PRIX_ACHAT_PRODUIT = rs_tDBInput_2.getBigDecimal(3);
            if(rs_tDBInput_2.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tDBInput_2 < 4) {
								row5.PRIX_VENTE_PRODUIT = null;
							} else {
		                          
            row5.PRIX_VENTE_PRODUIT = rs_tDBInput_2.getBigDecimal(4);
            if(rs_tDBInput_2.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tDBInput_2 < 5) {
								row5.CD_SOUS_CATEGORIE = null;
							} else {
	                         		
        	row5.CD_SOUS_CATEGORIE = routines.system.JDBCUtil.getString(rs_tDBInput_2, 5, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 6) {
								row5.LB_NOM_FICHIER = null;
							} else {
	                         		
        	row5.LB_NOM_FICHIER = routines.system.JDBCUtil.getString(rs_tDBInput_2, 6, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 7) {
								row5.DT_INSERTION = null;
							} else {
										
			row5.DT_INSERTION = routines.system.JDBCUtil.getDate(rs_tDBInput_2, 7);
		                    }
							if(colQtyInRs_tDBInput_2 < 8) {
								row5.BL_LIGNE_ACTIVE = 0;
							} else {
		                          
            row5.BL_LIGNE_ACTIVE = rs_tDBInput_2.getInt(8);
            if(rs_tDBInput_2.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
        	...
        	row5.LB_JOB_NAME = routines.system.JDBCUtil.getString(rs_tDBInput_2, 9, false);
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row5";
				...
				row5_HashRow.CD_PRODUIT = row5.CD_PRODUIT;
				
				row5_HashRow.NOM_PRODUIT = row5.NOM_PRODUIT;
				
				row5_HashRow.PRIX_ACHAT_PRODUIT = row5.PRIX_ACHAT_PRODUIT;
				
				row5_HashRow.PRIX_VENTE_PRODUIT = row5.PRIX_VENTE_PRODUIT;
				
				row5_HashRow.CD_SOUS_CATEGORIE = row5.CD_SOUS_CATEGORIE;
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jOdsProduit.class.getClassLoader().getResourceAsStream("adw/jodsproduit_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
...
/************************************************************************************************
 *     406142 characters generated by Talend Open Studio for Data Integration 
 *     on the 3 juin 2025 à 3:25:21 AM WAT
 ************************************************************************************************/