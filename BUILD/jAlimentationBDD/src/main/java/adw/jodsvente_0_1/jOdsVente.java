...
public String getFieldSeparator(){
	...
	private final String projectName = "ADW";
...
public static class row_Implicit_Context_DatabaseStruct implements routines.system.IPersistableRow<row_Implicit_Context_DatabaseStruct> {
    final static byte[] commonByteArrayLock_ADW_jOdsVente = new byte[0];
    static byte[] commonByteArray_ADW_jOdsVente = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_ADW_jOdsVente.length) {
				if(length < 1024 && commonByteArray_ADW_jOdsVente.length == 0) {
   					commonByteArray_ADW_jOdsVente = new byte[1024];
				} else {
   					commonByteArray_ADW_jOdsVente = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_ADW_jOdsVente, 0, length);
			strReturn = new String(commonByteArray_ADW_jOdsVente, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_ADW_jOdsVente, 0, length);
			strReturn = new String(commonByteArray_ADW_jOdsVente, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_ADW_jOdsVente) {
 ...
 */
	...
	final String decryptedPassword_Implicit_Context_Database = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:4MRORTqVTP/1es/SPIUihEex2v0ZFS5EFfTBqIzFF79qSMe7");
				
				String dbPwd_Implicit_Context_Database = decryptedPassword_Implicit_Context_Database;
				
				String url_Implicit_Context_Database = "jdbc:postgresql://" + "localhost" + ":" + "5432" + "/" + "vente_db";
		    ...
		    String dbquery_Implicit_Context_Database = "SELECT \n  \"PARAMS_LOG\".\"CONTEXTE\".\"key\", \n  \"PARAMS_LOG\".\"CONTEXTE\".\"value\"\nFROM \"PARAMS_LOG\".\"CONTEXTE"
+"\"";
 ...
 */
	...
	System.err.println("Le répertoire "+TalendDate.getDate("YYYYMMDD")+" ne contient pas de fichier Catégorie. Veuillez déposer le fichier du jour et relancer l'intégration");
	
	globalMap.put("tDie_1_DIE_MESSAGE", "Le répertoire "+TalendDate.getDate("YYYYMMDD")+" ne contient pas de fichier Catégorie. Veuillez déposer le fichier du jour et relancer l'intégration");
	globalMap.put("tDie_1_DIE_MESSAGES", "Le répertoire "+TalendDate.getDate("YYYYMMDD")+" ne contient pas de fichier Catégorie. Veuillez déposer le fichier du jour et relancer l'intégration");
...
public static class verifClientVenteStruct implements routines.system.IPersistableRow<verifClientVenteStruct> {
    final static byte[] commonByteArrayLock_ADW_jOdsVente = new byte[0];
    static byte[] commonByteArray_ADW_jOdsVente = new byte[0];
     ...
     * Compare keys
     */
    public int compareTo(verifClientVenteStruct other) {
...
public static class verifProduitVenteStruct implements routines.system.IPersistableRow<verifProduitVenteStruct> {
    final static byte[] commonByteArrayLock_ADW_jOdsVente = new byte[0];
    static byte[] commonByteArray_ADW_jOdsVente = new byte[0];
     ...
     * Compare keys
     */
    public int compareTo(verifProduitVenteStruct other) {
...
public static class insert_RejectStruct implements routines.system.IPersistableRow<insert_RejectStruct> {
    final static byte[] commonByteArrayLock_ADW_jOdsVente = new byte[0];
    static byte[] commonByteArray_ADW_jOdsVente = new byte[0];
	protected static final int DEFAULT_HASHCODE = 1;
    protected static final int PRIME = 31;
	...
	public boolean equals(Object obj) {
		...
		if (obj == null) return false;
		if (getClass() != obj.getClass()) return false;
		final insert_RejectStruct other = (insert_RejectStruct) obj;
	...
	public void copyDataTo(insert_RejectStruct other) {
	...
	public void copyKeysDataTo(insert_RejectStruct other) {
     ...
     * Compare keys
     */
    public int compareTo(insert_RejectStruct other) {
...
verifProduitVenteStruct verifProduitVente = new verifProduitVenteStruct();
verifClientVenteStruct verifClientVente = new verifClientVenteStruct();
...
insert_RejectStruct insert_Reject = new insert_RejectStruct();
 ...
 */
	...
	currentComponent="tMap_6";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"verifClientVente");
					}
 ...
 */
	...
	currentComponent="tMap_5";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"verifProduitVente");
					}
...
// ###############################
// # Outputs initialization
verifClientVenteStruct verifClientVente_tmp = new verifClientVenteStruct();
 ...
 */
	...
	currentComponent="tDBOutput_2";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"insert_Reject");
					}
...
// ###############################
// # Outputs initialization
verifProduitVenteStruct verifProduitVente_tmp = new verifProduitVenteStruct();
insert_RejectStruct insert_Reject_tmp = new insert_RejectStruct();
// ###############################
      ...
      nb_line_ok_tFilterRow_1++;
            ...
            if (row6 == null){
              row6 = new row6Struct();
                ...
                row6.ID_VENTE = ventes.ID_VENTE;
                row6.CD_PRODUIT = ventes.CD_PRODUIT;
                row6.DT_VENTE = ventes.DT_VENTE;
                row6.ID_CLIENT = ventes.ID_CLIENT;
                row6.QTE_VENTE = ventes.QTE_VENTE;
                row6.PRIX_ACHAT = ventes.PRIX_ACHAT;
                row6.PRIX_VENTE = ventes.PRIX_VENTE;
                row6.NUM_LIGNE = ventes.NUM_LIGNE;
...
if(row7 == null){ 
	...
	row7 = new row7Struct();
}				row7.ID_VENTE = row2.ID_VENTE;			row7.CD_PRODUIT = row2.CD_PRODUIT;			row7.DT_VENTE = row2.DT_VENTE;			row7.ID_CLIENT = row2.ID_CLIENT;			row7.QTE_VENTE = row2.QTE_VENTE;			row7.PRIX_ACHAT = row2.PRIX_ACHAT;			row7.PRIX_VENTE = row2.PRIX_VENTE;			row7.NUM_LIGNE = row2.NUM_LIGNE;			
...
verifProduitVente = null;
insert_Reject = null;

if(!rejectedInnerJoin_tMap_4 ) {

// # Output table : 'verifProduitVente'
verifProduitVente_tmp.ID_VENTE = row3.ID_VENTE ;
verifProduitVente_tmp.CD_PRODUIT = row3.CD_PRODUIT ;
verifProduitVente_tmp.DT_VENTE = row3.DT_VENTE ;
verifProduitVente_tmp.ID_CLIENT = row3.ID_CLIENT ;
verifProduitVente_tmp.QTE_VENTE = row3.QTE_VENTE ;
verifProduitVente_tmp.PRIX_ACHAT = row3.PRIX_ACHAT ;
verifProduitVente_tmp.PRIX_VENTE = row3.PRIX_VENTE ;
verifProduitVente_tmp.NUM_LIGNE = row3.NUM_LIGNE ;
verifProduitVente = verifProduitVente_tmp;
} // closing inner join bracket (1)
// ###### START REJECTS ##### 

// # Output reject table : 'insert_Reject'
// # Filter conditions 
if( rejectedInnerJoin_tMap_4 ) {
insert_Reject_tmp.LB_CHEMIN_FICHIER = ((String)globalMap.get("tFileList_1_CURRENT_FILEPATH"));
insert_Reject_tmp.LB_NOM_FICHIER = ((String)globalMap.get("tFileList_1_CURRENT_FILE"));
insert_Reject_tmp.NUM_LIGNE_REJET = row3.NUM_LIGNE ;
insert_Reject_tmp.LB_LIGNE_REJET = row3.ID_VENTE +'|'+ row3.CD_PRODUIT +'|'+  row3.DT_VENTE +'|'+ row3.ID_CLIENT ;
insert_Reject_tmp.LB_MESSAGE_REJET = "Dependance au Produit : " +  row3.CD_PRODUIT + " n'exsite pas dans la table ODS_PRODUIT" ;
insert_Reject_tmp.LB_NOM_FLUX = ((String)globalMap.get("nom_flux"));
insert_Reject_tmp.DT_REJET = TalendDate.getCurrentDate() ;
insert_Reject_tmp.LB_JOB_NAME = jobName;
insert_Reject = insert_Reject_tmp;
 ...
 */
// Start of branch "verifProduitVente"
if(verifProduitVente != null) { 
 					    ...
 					    boolean forceLooprow5 = false;
 							...
 							row5Struct row5ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_5 = false;
                        		    		    ...
                        		    		    row5HashKey.ID_CLIENT = verifProduitVente.ID_CLIENT ;
...
verifClientVente = null;
clientInexistant = null;

if(!rejectedInnerJoin_tMap_5 ) {

// # Output table : 'verifClientVente'
verifClientVente_tmp.ID_VENTE = verifProduitVente.ID_VENTE ;
verifClientVente_tmp.CD_PRODUIT = verifProduitVente.CD_PRODUIT ;
verifClientVente_tmp.DT_VENTE = verifProduitVente.DT_VENTE ;
verifClientVente_tmp.ID_CLIENT = verifProduitVente.ID_CLIENT ;
verifClientVente_tmp.QTE_VENTE = verifProduitVente.QTE_VENTE ;
verifClientVente_tmp.PRIX_ACHAT = verifProduitVente.PRIX_ACHAT ;
verifClientVente_tmp.PRIX_VENTE = verifProduitVente.PRIX_VENTE ;
verifClientVente_tmp.NUM_LIGNE = verifProduitVente.NUM_LIGNE ;
verifClientVente = verifClientVente_tmp;
} // closing inner join bracket (1)
// ###### START REJECTS ##### 
...
clientInexistant_tmp.NUM_LIGNE_REJET = verifProduitVente.NUM_LIGNE ;
clientInexistant_tmp.LB_LIGNE_REJET = verifProduitVente.ID_VENTE +'|'+ verifProduitVente.CD_PRODUIT +'|'+  verifProduitVente.DT_VENTE  +'|'+ verifProduitVente.ID_CLIENT  +'|'+ verifProduitVente.QTE_VENTE ;
clientInexistant_tmp.LB_MESSAGE_REJET = "Dependance au Client ==> "+ verifProduitVente.ID_CLIENT +" n'existe pas dan sla table ODS_CLIENT " ;
 ...
 */
// Start of branch "verifClientVente"
if(verifClientVente != null) { 
 					    ...
 					    boolean forceLooprow8 = false;
 							...
 							row8Struct row8ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_6 = false;
                        		    		    ...
                        		    		    row8HashKey.ID_VENTE = verifClientVente.ID_VENTE ;
                        		    		
                        		    		    row8HashKey.CD_PRODUIT = verifClientVente.CD_PRODUIT ;
...
venteExistant_tmp.NUM_LIGNE_REJET = verifClientVente.NUM_LIGNE ;
venteExistant_tmp.LB_LIGNE_REJET = verifClientVente.ID_VENTE +'|'+  verifClientVente.CD_PRODUIT  +'|'+  verifClientVente.DT_VENTE   +'|'+ verifClientVente.ID_CLIENT ;
venteExistant_tmp.LB_MESSAGE_REJET = "donnes deja inseree dans la table";
...
// # Filter conditions 
if( rejectedInnerJoin_tMap_6 ) {
insertVente_tmp.ID_VENTE = verifClientVente.ID_VENTE ;
insertVente_tmp.CD_PRODUIT = verifClientVente.CD_PRODUIT ;
insertVente_tmp.DT_VENTE = verifClientVente.DT_VENTE ;
insertVente_tmp.ID_CLIENT = verifClientVente.ID_CLIENT ;
insertVente_tmp.QTE_VENTE = verifClientVente.QTE_VENTE ;
insertVente_tmp.PRIX_ACHAT = verifClientVente.PRIX_ACHAT ;
insertVente_tmp.PRIX_VENTE = verifClientVente.PRIX_VENTE ;
...
} else {pstmtUpdate_tDBOutput_6.setString(1, venteExistant.LB_MESSAGE_REJET);
}

                        if(venteExistant.LB_NOM_FLUX == null) {
pstmtUpdate_tDBOutput_6.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(2, venteExistant.LB_NOM_FLUX);
}

                        if(venteExistant.DT_REJET != null) {
pstmtUpdate_tDBOutput_6.setTimestamp(3, new java.sql.Timestamp(venteExistant.DT_REJET.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_6.setString(4, venteExistant.LB_JOB_NAME);
}

                        if(venteExistant.LB_CHEMIN_FICHIER == null) {
pstmtUpdate_tDBOutput_6.setNull(5 + count_tDBOutput_6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(5 + count_tDBOutput_6, venteExistant.LB_CHEMIN_FICHIER);
}

                        if(venteExistant.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_6.setNull(6 + count_tDBOutput_6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(6 + count_tDBOutput_6, venteExistant.LB_NOM_FICHIER);
}

                        pstmtUpdate_tDBOutput_6.setInt(7 + count_tDBOutput_6, venteExistant.NUM_LIGNE_REJET);

                        if(venteExistant.LB_LIGNE_REJET == null) {
pstmtUpdate_tDBOutput_6.setNull(8 + count_tDBOutput_6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_6.setString(8 + count_tDBOutput_6, venteExistant.LB_LIGNE_REJET);
}
...
} else {pstmtInsert_tDBOutput_6.setString(1, venteExistant.LB_CHEMIN_FICHIER);
}

                        if(venteExistant.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_6.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(2, venteExistant.LB_NOM_FICHIER);
}

                        pstmtInsert_tDBOutput_6.setInt(3, venteExistant.NUM_LIGNE_REJET);

                        if(venteExistant.LB_LIGNE_REJET == null) {
pstmtInsert_tDBOutput_6.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(4, venteExistant.LB_LIGNE_REJET);
}

                        if(venteExistant.LB_MESSAGE_REJET == null) {
pstmtInsert_tDBOutput_6.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(5, venteExistant.LB_MESSAGE_REJET);
}

                        if(venteExistant.LB_NOM_FLUX == null) {
pstmtInsert_tDBOutput_6.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_6.setString(6, venteExistant.LB_NOM_FLUX);
}

                        if(venteExistant.DT_REJET != null) {
pstmtInsert_tDBOutput_6.setTimestamp(7, new java.sql.Timestamp(venteExistant.DT_REJET.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_6.setString(8, venteExistant.LB_JOB_NAME);
}
...
} // End of branch "verifClientVente"
...
} else {pstmt_tDBOutput_3.setString(1, clientInexistant.LB_CHEMIN_FICHIER);
}

                    if(clientInexistant.LB_NOM_FICHIER == null) {
pstmt_tDBOutput_3.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_3.setString(2, clientInexistant.LB_NOM_FICHIER);
}

                    pstmt_tDBOutput_3.setInt(3, clientInexistant.NUM_LIGNE_REJET);

                    if(clientInexistant.LB_LIGNE_REJET == null) {
pstmt_tDBOutput_3.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_3.setString(4, clientInexistant.LB_LIGNE_REJET);
}
...
} else {pstmtUpdate_tDBOutput_3.setString(1, clientInexistant.LB_MESSAGE_REJET);
}

                        if(clientInexistant.LB_NOM_FLUX == null) {
pstmtUpdate_tDBOutput_3.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(2, clientInexistant.LB_NOM_FLUX);
}

                        if(clientInexistant.DT_REJET != null) {
pstmtUpdate_tDBOutput_3.setTimestamp(3, new java.sql.Timestamp(clientInexistant.DT_REJET.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_3.setString(4, clientInexistant.LB_JOB_NAME);
}

                        if(clientInexistant.LB_CHEMIN_FICHIER == null) {
pstmtUpdate_tDBOutput_3.setNull(5 + count_tDBOutput_3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(5 + count_tDBOutput_3, clientInexistant.LB_CHEMIN_FICHIER);
}

                        if(clientInexistant.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_3.setNull(6 + count_tDBOutput_3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(6 + count_tDBOutput_3, clientInexistant.LB_NOM_FICHIER);
}

                        pstmtUpdate_tDBOutput_3.setInt(7 + count_tDBOutput_3, clientInexistant.NUM_LIGNE_REJET);

                        if(clientInexistant.LB_LIGNE_REJET == null) {
pstmtUpdate_tDBOutput_3.setNull(8 + count_tDBOutput_3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(8 + count_tDBOutput_3, clientInexistant.LB_LIGNE_REJET);
}
...
} else {pstmtInsert_tDBOutput_3.setString(1, clientInexistant.LB_CHEMIN_FICHIER);
}

                        if(clientInexistant.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_3.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(2, clientInexistant.LB_NOM_FICHIER);
}

                        pstmtInsert_tDBOutput_3.setInt(3, clientInexistant.NUM_LIGNE_REJET);

                        if(clientInexistant.LB_LIGNE_REJET == null) {
pstmtInsert_tDBOutput_3.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(4, clientInexistant.LB_LIGNE_REJET);
}

                        if(clientInexistant.LB_MESSAGE_REJET == null) {
pstmtInsert_tDBOutput_3.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(5, clientInexistant.LB_MESSAGE_REJET);
}

                        if(clientInexistant.LB_NOM_FLUX == null) {
pstmtInsert_tDBOutput_3.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(6, clientInexistant.LB_NOM_FLUX);
}

                        if(clientInexistant.DT_REJET != null) {
pstmtInsert_tDBOutput_3.setTimestamp(7, new java.sql.Timestamp(clientInexistant.DT_REJET.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_3.setString(8, clientInexistant.LB_JOB_NAME);
}
...
} // End of branch "verifProduitVente"




// Start of branch "insert_Reject"
if(insert_Reject != null) { 
	...
	currentComponent="tDBOutput_2";
        ...
        whetherReject_tDBOutput_2 = false;
                    if(insert_Reject.LB_CHEMIN_FICHIER == null) {
pstmt_tDBOutput_2.setNull(1, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(1, insert_Reject.LB_CHEMIN_FICHIER);
}

                    if(insert_Reject.LB_NOM_FICHIER == null) {
pstmt_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(2, insert_Reject.LB_NOM_FICHIER);
}

                    pstmt_tDBOutput_2.setInt(3, insert_Reject.NUM_LIGNE_REJET);

                    if(insert_Reject.LB_LIGNE_REJET == null) {
pstmt_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(4, insert_Reject.LB_LIGNE_REJET);
}
            ...
            if(checkCount_tDBOutput_2 > 0) {
                        if(insert_Reject.LB_MESSAGE_REJET == null) {
pstmtUpdate_tDBOutput_2.setNull(1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(1, insert_Reject.LB_MESSAGE_REJET);
}

                        if(insert_Reject.LB_NOM_FLUX == null) {
pstmtUpdate_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(2, insert_Reject.LB_NOM_FLUX);
}

                        if(insert_Reject.DT_REJET != null) {
pstmtUpdate_tDBOutput_2.setTimestamp(3, new java.sql.Timestamp(insert_Reject.DT_REJET.getTime()));
} else {
pstmtUpdate_tDBOutput_2.setNull(3, java.sql.Types.TIMESTAMP);
}

                        if(insert_Reject.LB_JOB_NAME == null) {
pstmtUpdate_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(4, insert_Reject.LB_JOB_NAME);
}

                        if(insert_Reject.LB_CHEMIN_FICHIER == null) {
pstmtUpdate_tDBOutput_2.setNull(5 + count_tDBOutput_2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(5 + count_tDBOutput_2, insert_Reject.LB_CHEMIN_FICHIER);
}

                        if(insert_Reject.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_2.setNull(6 + count_tDBOutput_2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(6 + count_tDBOutput_2, insert_Reject.LB_NOM_FICHIER);
}

                        pstmtUpdate_tDBOutput_2.setInt(7 + count_tDBOutput_2, insert_Reject.NUM_LIGNE_REJET);

                        if(insert_Reject.LB_LIGNE_REJET == null) {
pstmtUpdate_tDBOutput_2.setNull(8 + count_tDBOutput_2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(8 + count_tDBOutput_2, insert_Reject.LB_LIGNE_REJET);
}
...
} else {pstmtInsert_tDBOutput_2.setString(1, insert_Reject.LB_CHEMIN_FICHIER);
}

                        if(insert_Reject.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(2, insert_Reject.LB_NOM_FICHIER);
}

                        pstmtInsert_tDBOutput_2.setInt(3, insert_Reject.NUM_LIGNE_REJET);

                        if(insert_Reject.LB_LIGNE_REJET == null) {
pstmtInsert_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(4, insert_Reject.LB_LIGNE_REJET);
}

                        if(insert_Reject.LB_MESSAGE_REJET == null) {
pstmtInsert_tDBOutput_2.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(5, insert_Reject.LB_MESSAGE_REJET);
}

                        if(insert_Reject.LB_NOM_FLUX == null) {
pstmtInsert_tDBOutput_2.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(6, insert_Reject.LB_NOM_FLUX);
}

                        if(insert_Reject.DT_REJET != null) {
pstmtInsert_tDBOutput_2.setTimestamp(7, new java.sql.Timestamp(insert_Reject.DT_REJET.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_2.setString(8, insert_Reject.LB_JOB_NAME);
}
...
insertDuplicate_tmp.NUM_LIGNE_REJET = row7.NUM_LIGNE ;
insertDuplicate_tmp.LB_LIGNE_REJET = row7.ID_VENTE +'|'+ row7.CD_PRODUIT +'|'+ row7.DT_VENTE +'|'+ row7.ID_CLIENT ;
insertDuplicate_tmp.LB_MESSAGE_REJET = "donnees En doubles";
...
} else {pstmt_tDBOutput_1.setString(1, insertDuplicate.LB_CHEMIN_FICHIER);
}

                    if(insertDuplicate.LB_NOM_FICHIER == null) {
pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(2, insertDuplicate.LB_NOM_FICHIER);
}

                    pstmt_tDBOutput_1.setInt(3, insertDuplicate.NUM_LIGNE_REJET);

                    if(insertDuplicate.LB_LIGNE_REJET == null) {
pstmt_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(4, insertDuplicate.LB_LIGNE_REJET);
}
...
} else {pstmtUpdate_tDBOutput_1.setString(1, insertDuplicate.LB_MESSAGE_REJET);
}

                        if(insertDuplicate.LB_NOM_FLUX == null) {
pstmtUpdate_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(2, insertDuplicate.LB_NOM_FLUX);
}

                        if(insertDuplicate.DT_REJET != null) {
pstmtUpdate_tDBOutput_1.setTimestamp(3, new java.sql.Timestamp(insertDuplicate.DT_REJET.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_1.setString(4, insertDuplicate.LB_JOB_NAME);
}

                        if(insertDuplicate.LB_CHEMIN_FICHIER == null) {
pstmtUpdate_tDBOutput_1.setNull(5 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(5 + count_tDBOutput_1, insertDuplicate.LB_CHEMIN_FICHIER);
}

                        if(insertDuplicate.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_1.setNull(6 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(6 + count_tDBOutput_1, insertDuplicate.LB_NOM_FICHIER);
}

                        pstmtUpdate_tDBOutput_1.setInt(7 + count_tDBOutput_1, insertDuplicate.NUM_LIGNE_REJET);

                        if(insertDuplicate.LB_LIGNE_REJET == null) {
pstmtUpdate_tDBOutput_1.setNull(8 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(8 + count_tDBOutput_1, insertDuplicate.LB_LIGNE_REJET);
}
...
} else {pstmtInsert_tDBOutput_1.setString(1, insertDuplicate.LB_CHEMIN_FICHIER);
}

                        if(insertDuplicate.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(2, insertDuplicate.LB_NOM_FICHIER);
}

                        pstmtInsert_tDBOutput_1.setInt(3, insertDuplicate.NUM_LIGNE_REJET);

                        if(insertDuplicate.LB_LIGNE_REJET == null) {
pstmtInsert_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(4, insertDuplicate.LB_LIGNE_REJET);
}

                        if(insertDuplicate.LB_MESSAGE_REJET == null) {
pstmtInsert_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(5, insertDuplicate.LB_MESSAGE_REJET);
}

                        if(insertDuplicate.LB_NOM_FLUX == null) {
pstmtInsert_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(6, insertDuplicate.LB_NOM_FLUX);
}

                        if(insertDuplicate.DT_REJET != null) {
pstmtInsert_tDBOutput_1.setTimestamp(7, new java.sql.Timestamp(insertDuplicate.DT_REJET.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_1.setString(8, insertDuplicate.LB_JOB_NAME);
}
...
insertReject_tmp.NUM_LIGNE_REJET = row6.NUM_LIGNE ;
insertReject_tmp.LB_LIGNE_REJET = row6.ID_VENTE +'|'+ row6.CD_PRODUIT +'|'+ row6.DT_VENTE +'|'+ row6.ID_CLIENT ;
...
// ###############################      
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"verifProduitVente");
			  	}
...
// ###############################      
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"verifClientVente");
			  	}
 ...
 */
	...
	nb_line_rejected_tDBOutput_2=nb_line_rejected_tDBOutput_2 + rejectedCount_tDBOutput_2;
        ...
        globalMap.put("tDBOutput_2_NB_LINE_REJECTED", nb_line_rejected_tDBOutput_2);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"insert_Reject");
			  	}
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row4";
		...
		int tos_count_tAdvancedHash_row4 = 0;
			   		...
			   		// linked node: tMap_4 - inputs:(row3,row4) outputs:(verifProduitVente,insert_Reject)
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row5";
		...
		int tos_count_tAdvancedHash_row5 = 0;
			   		...
			   		// source node:tDBInput_2 - inputs:(after_tFileInputDelimited_1) outputs:(row5,row5) | target node:tAdvancedHash_row5 - inputs:(row5) outputs:()
			   		// linked node: tMap_5 - inputs:(verifProduitVente,row5) outputs:(verifClientVente,clientInexistant)
 ...
 */
	...
	currentComponent="tDBInput_2";
		...
		int tos_count_tDBInput_2 = 0;
        	...
        	row5.ID_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 1, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 2) {
								row5.NOM_CLIENT = null;
							} else {
	                         		
        	row5.NOM_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 3) {
								row5.PREN_CLIENT = null;
							} else {
	                         		
        	row5.PREN_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 3, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 4) {
								row5.CD_POSTAL_CLIENT = null;
							} else {
	                         		
        	row5.CD_POSTAL_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 4, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 5) {
								row5.VILLE_CLIENT = null;
							} else {
	                         		
        	row5.VILLE_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 5, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 6) {
								row5.PAYS_CLIENT = null;
							} else {
	                         		
        	row5.PAYS_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 6, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 7) {
								row5.REGION_CLIENT = null;
							} else {
	                         		
        	row5.REGION_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 7, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 8) {
								row5.CD_TYPE_CLIENT = null;
							} else {
	                         		
        	row5.CD_TYPE_CLIENT = routines.system.JDBCUtil.getString(rs_tDBInput_2, 8, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 9) {
								row5.LB_NOM_FICHIER = null;
							} else {
	                         		
        	row5.LB_NOM_FICHIER = routines.system.JDBCUtil.getString(rs_tDBInput_2, 9, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 10) {
								row5.DT_INSERTION = null;
							} else {
										
			row5.DT_INSERTION = routines.system.JDBCUtil.getDate(rs_tDBInput_2, 10);
		                    }
							if(colQtyInRs_tDBInput_2 < 11) {
								row5.BL_LIGNE_ACTIVE = 0;
							} else {
		                          
            row5.BL_LIGNE_ACTIVE = rs_tDBInput_2.getInt(11);
        	...
        	row5.LB_JOB_NAME = routines.system.JDBCUtil.getString(rs_tDBInput_2, 12, false);
		                    }
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row8";
		...
		int tos_count_tAdvancedHash_row8 = 0;
			   		...
			   		// linked node: tMap_6 - inputs:(verifClientVente,row8) outputs:(insertVente,venteExistant)
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jOdsVente.class.getClassLoader().getResourceAsStream("adw/jodsvente_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
...
/************************************************************************************************
 *     473670 characters generated by Talend Open Studio for Data Integration 
 *     on the 3 juin 2025 à 3:25:22 AM WAT
 ************************************************************************************************/